...
class serverMUD:
    ...
    async def start_moving(self):
        """Initiate monsters' moving process."""
        while True:
            await asyncio.sleep(30)
            try:
                await self.move_monster()
            except Exception as e:
                print(f"Moving error: {e}")

    async def move_monster(self):
        """Move random monster in random direction."""
        directions = [("right", 1, 0), ("left", -1, 0), ("down", 0, 1), ("up", 0, -1)]
        m = []
        for x in range(self.WIDTH):
            for y in range(self.HEIGHT):
                if self.monsters[x][y] is not None:
                    m.append((x, y, self.monsters[x][y]))
        ...
        for x, y, data in m:
            ...
            for direc, dx, dy in directions:
                new_x = (x + dx + self.WIDTH) % self.WIDTH
                new_y = (y + dy + self.HEIGHT) % self.HEIGHT
                if self.monsters[new_x][new_y] is None:
                    self.monsters[new_x][new_y] = data
                    self.monsters[x][y] = None

                    for q, px, py in self.clients.values():
                        await q.put(f"monster_moved {name} {direc}\n")
                        if px == new_x and py == new_y:
                            await q.put(f"encountered {name} '{hello}'\n")
...
async def server_main(port):
    """Launch server."""
    MUD.move_task = asyncio.create_task(MUD.start_moving())
